...
describe('TITAN plan generator',()=>{
 ...
 it('expõe uma recomendação e score calculados pela Engine',()=>{const candidates=generateTitanPlanCandidates(profile,assessment);const recommended=candidates.filter(c=>c.recommended);expect(recommended).toHaveLength(1);expect(recommended[0].titanScore).toBeGreaterThanOrEqual(0);expect(recommended[0].titanScore).toBeLessThanOrEqual(100);expect(recommended[0].engineMetrics?.volumeTargetCoverage).toBeDefined();expect(recommended[0].rationale.some(r=>r.includes('Score TITAN'))).toBe(true);});
});
