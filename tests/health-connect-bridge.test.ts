...
describe('Health Connect bridge', () => {
  ...
  it('usa o plugin Capacitor quando instalado no app Android', async () => {
    ...
    const requestPermissions = vi.fn().mockResolvedValue({ granted: true });
    ...
    window.Capacitor = { Plugins: { TitanHealthConnect: { isAvailable, requestPermissions, readSamples: readSamplesNative } } };
    ...
    expect(requestPermissions).toHaveBeenCalledWith({ types: ['steps'] });
