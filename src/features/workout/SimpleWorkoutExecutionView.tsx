...
export function SimpleWorkoutExecutionView({ planId, planName, workout, onBack, onCompleted }: Props) {
  ...
  function resetSession() {
    ...
    setRestRunning(false); setRestSeconds(0); setSessionSeconds(0);
  }

  if (summary) return <section className="workout-summary"><span className="eyebrow">TREINO CONCLUÍDO</span><h2>{workout.title}</h2><p>Treino registrado com sucesso.</p><div className="summary-grid"><div><span>Tempo</span><strong>{formatDuration(summary.durationSeconds)}</strong></div><div><span>Séries concluídas</span><strong>{summary.totalSets}</strong></div></div><button type="button" className="primary-action" onClick={onCompleted}>Finalizar</button></section>;

  return <div className="workout-mode simple-workout-mode">
    <button type="button" className="secondary-action back-action" onClick={onBack}>← Sair do modo treino</button>
    <section className="workout-progress-card compact-workout-progress"><div><span className="eyebrow">EXERCÍCIO {activeExerciseIndex + 1} DE {workout.exercises.length}</span><h2>{workout.title}</h2><p>{completedSets} de {totalSets} séries concluídas · Tempo {formatSessionTime(sessionSeconds)}</p></div><strong>{progress}%</strong><div className="workout-progress-track"><span style={{ width: `${progress}%` }} /></div></section>

    {restSeconds > 0 && <section className="rest-timer active" aria-label="Cronômetro de descanso"><div><span className="info-label">DESCANSO</span><strong>{formatTimer(restSeconds)}</strong></div><div className="timer-actions"><button type="button" className="secondary-action" onClick={() => setRestRunning((value) => !value)}>{restRunning ? 'Pausar' : 'Continuar'}</button><button type="button" className="secondary-action" onClick={() => { setRestRunning(false); setRestSeconds(0); }}>Pular</button></div></section>}

    <article className="active-exercise-card simple-exercise-card">
      <header><span className="exercise-order">{activeExerciseIndex + 1}</span><div><span className="info-label">{exercise.muscleGroup}</span><h3>{exercise.name}</h3></div></header>
      <div className="exercise-prescription simple-prescription"><span><strong>{exerciseExecution.sets.length} {exerciseExecution.sets.length === 1 ? 'série' : 'séries'}</strong></span>{exercise.restSeconds ? <span><strong>Descanso {formatRest(exercise.restSeconds)}</strong></span> : null}{exercise.minReps || exercise.maxReps ? <span><strong>{formatReps(exercise)} reps</strong></span> : null}{exercise.durationSeconds ? <span><strong>{formatPlannedDuration(exercise.durationSeconds)}</strong></span> : null}</div>
      {exercise.technique && <p className="exercise-cue">{exercise.technique}</p>}
      <div className="set-entry-list simple-set-list">{exerciseExecution.sets.map((set) => <div className={`set-entry simple-set-entry ${set.completed ? 'completed' : ''}`} key={set.setNumber}><div className="set-entry-title"><strong>Série {set.setNumber} de {exerciseExecution.sets.length}</strong><span>{set.completed ? 'Concluída' : 'Pendente'}</span></div><button type="button" className="complete-set-action" aria-pressed={set.completed} onClick={() => toggleSet(set.setNumber)}>{set.completed ? '✓ Série registrada' : `Registrar série ${set.setNumber}`}</button></div>)}</div>
    </article>

    <div className="exercise-navigation"><button type="button" className="secondary-action" disabled={activeExerciseIndex === 0} onClick={previousExercise}>Anterior</button>{activeExerciseIndex < workout.exercises.length - 1 ? <button type="button" className="primary-action" disabled={!exerciseCompleted} onClick={nextExercise}>Próximo exercício</button> : <button type="button" className="primary-action" disabled={completedSets !== totalSets} onClick={finishWorkout}>Concluir treino</button>}</div>
...
function createHistoryRecord(planId:string, planName:string, workout:TitanWorkoutDay, execution:WorkoutExecution, completedAt:string):WorkoutHistoryRecord { const exercises=workout.exercises.map((exercise)=>{ const sets=execution.exercises[exercise.id].sets.filter((set)=>set.completed).map(({ completed:_completed, ...set })=>set); return { exerciseId:exercise.id, name:exercise.name, muscleGroup:exercise.muscleGroup, exerciseType:exercise.exerciseType ?? 'strength', sets, volumeKg:0, bestWeightKg:null, totalDistanceMeters:0, totalDurationSeconds:0, bestSpeedKmh:null, bestInclinePercent:null, averageHeartRate:null }; }); const startedAt=execution.timerStartedAt ?? completedAt; return { id:`${planId}:${workout.id}:${completedAt}`, planId, planName, workoutId:workout.id, workoutTitle:workout.title, workoutDay:workout.day, startedAt, completedAt, durationSeconds:Math.max(0,Math.round((new Date(completedAt).getTime()-new Date(startedAt).getTime())/1000)), totalSets:exercises.reduce((sum,item)=>sum+item.sets.length,0), totalVolumeKg:0, exercises }; }
...
function formatRest(seconds:number) { return seconds < 60 ? `${seconds}s` : `${Math.floor(seconds/60)}m${seconds%60 ? ` ${seconds%60}s` : ''}`; }
function formatPlannedDuration(seconds:number) { return seconds >= 60 ? `${Math.round(seconds/60)} min` : `${seconds}s`; }
