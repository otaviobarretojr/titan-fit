...
export function WorkoutExecutionView({ planId, planName, workout, onBack, onCompleted }: Props) {
  ...
  function completeSet(set: ExecutedSet) {
    ...
    if (completed) startRest(activeExercise.restSeconds);
  }
  ...
  function previousExercise() { setActiveExerciseIndex((value) => Math.max(0, value - 1)); window.scrollTo({ top: 0, behavior: 'smooth' }); }
  ...
  function resetSession() {
    ...
    removeWorkoutExecution(planId, workout.id); setExecution(createExecution(planId, workout)); setActiveExerciseIndex(0); setTimerRunning(false); setTimerSeconds(0);
  }

  if (summary) return <section className="workout-summary"><span className="eyebrow">TREINO CONCLUÍDO</span><h2>{workout.title}</h2><p>Excelente. Sua sessão foi salva no histórico.</p><div className="summary-grid"><div><span>Tempo</span><strong>{formatDuration(summary.durationSeconds)}</strong></div><div><span>Volume</span><strong>{Math.round(summary.totalVolumeKg).toLocaleString('pt-BR')} kg</strong></div><div><span>Séries</span><strong>{summary.totalSets}</strong></div><div><span>Cardio</span><strong>{summary.cardioMinutes ? `${summary.cardioMinutes} min` : '—'}</strong></div></div><button type="button" className="primary-action" onClick={onCompleted}>Ver progresso</button></section>;
  ...
  return <div className="workout-mode">
    ...
    <section className="workout-progress-card"><div><span className="eyebrow">MODO TREINO · EXERCÍCIO {activeExerciseIndex + 1} DE {workout.exercises.length}</span><h2>{workout.title}</h2><p>{totals.completed} de {totals.total} séries concluídas</p></div><strong>{progress}%</strong><div className="workout-progress-track"><span style={{ width: `${progress}%` }} /></div></section>
