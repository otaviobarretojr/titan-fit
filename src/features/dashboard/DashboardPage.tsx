...
function getTodayCardio(plan: TitanPlan): TitanCardioSession | null {
  const today = WEEKDAYS[new Date().getDay()];
  return plan.project?.cardioSchedule?.find((session) => normalize(session.day).includes(today)) ?? null;
}
