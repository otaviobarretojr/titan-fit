...
export function PlanViewer({ plan, initialWorkoutId, onImportAnother, onRemove, onHistoryChange, onExitWorkout, onDirectStartHandled }: Props) {
  ...
  return <><section className="section-header"><span className="eyebrow">PROJETO ATIVO</span><h2>{plan.project?.name ?? plan.name}</h2>{plan.description && <p>{plan.description}</p>}<p>{plan.workouts.length} treinos • {exerciseCount} exercícios</p></section>
    <section className="section-header"><span className="eyebrow">TREINOS</span><h2>Projeto completo</h2><p>{videoCount} vídeos vinculados</p></section>
...
function ExerciseDetail({ exercise, workout, onBack }: { exercise: TitanExercise; workout: TitanWorkoutDay; onBack: () => void }) {
  const [showVideo, setShowVideo] = useState(false); const type = exercise.exerciseType ?? 'strength';
  return <><button type="button" className="secondary-action back-action" onClick={onBack}>← Voltar para o treino</button><section className="exercise-detail-header"><span className="eyebrow">{workout.title.toUpperCase()}</span><h2>{exercise.name}</h2><p>{exercise.muscleGroup} · {typeLabel(type)}</p></section><section className="prescription-grid">{metrics(exercise).map((metric) => <Metric key={metric.label} {...metric} />)}</section>{exercise.video && <section className="video-card"><div className="video-card-heading"><div><span className="info-label">VÍDEO DE EXECUÇÃO</span><strong>{exercise.video.title ?? exercise.name}</strong></div><button type="button" className="secondary-action" onClick={() => setShowVideo((value) => !value)}>{showVideo ? 'Fechar vídeo' : 'Ver execução'}</button></div>{showVideo && <div className="video-frame"><iframe src={`https://www.youtube-nocookie.com/embed/${exercise.video.videoId}?rel=0`} title={exercise.video.title ?? exercise.name} loading="lazy" allowFullScreen /></div>}</section>}{exercise.technique && <DetailSection title="Técnica" text={exercise.technique} />}{exercise.commonMistakes?.length ? <ListSection title="Erros comuns" items={exercise.commonMistakes} /> : null}{exercise.progression?.length ? <section className="detail-card"><span className="info-label">PROGRESSÃO PLANEJADA</span>{exercise.progression.map((step) => <p key={`${step.startWeek}-${step.endWeek}`}><strong>Semanas {step.startWeek}–{step.endWeek}:</strong> {step.inclinePercent ?? '—'}% · {step.speedMinKmh ?? step.speedKmh ?? '—'}–{step.speedMaxKmh ?? step.speedKmh ?? '—'} km/h</p>)}</section> : null}</>;
}

function metrics(exercise: TitanExercise) {
  const type = exercise.exerciseType ?? 'strength'; const result = [] as Array<{ label: string; value: string }>;
  if (exercise.sets) result.push({ label: 'Séries', value: String(exercise.sets) });
  if (type === 'strength') result.push({ label: 'Repetições', value: formatRepetitions(exercise) }, { label: 'RIR', value: exercise.targetRir === undefined ? '—' : String(exercise.targetRir) });
  if (type === 'distance') result.push({ label: 'Distância', value: exercise.minDistanceMeters && exercise.maxDistanceMeters ? `${exercise.minDistanceMeters}–${exercise.maxDistanceMeters} m` : `${exercise.distanceMeters ?? '—'} m` });
  if (['cardio', 'isometric', 'mobility'].includes(type)) result.push({ label: 'Tempo', value: formatDuration(exercise.durationSeconds) });
  if (type === 'cardio' && exercise.inclinePercent !== undefined) result.push({ label: 'Inclinação', value: `${exercise.inclinePercent}%` });
  if (exercise.restSeconds !== undefined && exercise.restSeconds > 0) result.push({ label: 'Descanso', value: formatRest(exercise.restSeconds) });
  return result;
}
