...
export function PlanImporter({ onImport }: Props) {
  ...
  async function readFile(file: File) {
    ...
    if (!file.name.toLowerCase().endsWith('.json') && !file.name.toLowerCase().endsWith('.titan')) {
      setErrors(['Selecione um arquivo .json ou .titan.']);
      return;
    }
    if (file.size > 1_000_000) {
      setErrors(['O arquivo excede o limite de 1 MB.']);
      return;
    }
    ...
    try {
      ...
      setErrors(['Não foi possível ler o JSON. Verifique se o arquivo está bem formatado.']);
    }
  }
  ...
  return (
    <section className="import-card" aria-labelledby="import-title">
      <div>
        <span className="eyebrow">IMPORTAR FICHA</span>
        <h3 id="import-title">Arquivo TITAN FIT</h3>
        <p>Escolha um arquivo validado. Nada será substituído antes da confirmação.</p>
      </div>
      ...
      <input
        ...
        aria-label="Selecionar ficha TITAN FIT"
        onChange={(event) => {
      ...
      {preview && (
        <div className="plan-preview">
          ...
          <div className="preview-actions">
            <button type="button" className="primary-action" onClick={confirmImport}>Confirmar importação</button>
            <button type="button" className="secondary-action" onClick={() => setPreview(null)}>Cancelar</button>
          </div>
        </div>
      )}
    </section>
