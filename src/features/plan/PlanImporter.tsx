...
export function PlanImporter({ onImport }: Props) {
  ...
  async function readFile(file: File) {
    ...
    try {
      ...
      if (isCardioFile) {
        ...
        if (!activePlan) { setErrors(['Importe primeiro o Projeto Musculação. Depois volte aqui e selecione o Projeto Cardio.']); return; }
        ...
        setWarnings(['O Projeto Cardio será adicionado ao projeto ativo sem apagar a musculação nem o histórico existente.']);
        return;
      }
  ...
  function confirmImport() {
    if (!preview) return;
    onImport(preview); setPreview(null); setSummary(null); setFileName(''); setImportKind('project');
    if (inputRef.current) inputRef.current.value = '';
  }

  return <section className="import-card" aria-labelledby="import-title">
    <div><span className="eyebrow">INSERIR MEU PROJETO</span><h3 id="import-title">Projeto externo</h3><p>Importe Projeto Musculação ou Projeto Cardio. O TITAN valida o arquivo antes de ativar e preserva seus registros anteriores.</p></div>
    <div className="validation-message warning"><strong>Ordem recomendada</strong><ul><li>1. Importe primeiro o Projeto Musculação (.titan).</li><li>2. Depois importe o Projeto Cardio (.titan-cardio) nesta mesma tela.</li></ul></div>
    ...
    {preview&&summary&&<div className="plan-preview"><span className="eyebrow">PRÉVIA VALIDADA</span><h3>{importKind === 'cardio' ? 'Projeto Cardio' : (preview.project?.name??preview.name)}</h3><p>{importKind === 'cardio' ? <><strong>Será incorporado ao projeto ativo.</strong> A musculação será mantida.</> : <>Origem: <strong>Projeto importado</strong>{preview.project?.originalAuthor?` · ${preview.project.originalAuthor}`:''}</>}</p>
      ...
      <p className="profile-privacy-note">O Coach TITAN poderá analisar sua execução e progresso, mas mudanças sugeridas serão identificadas como recomendações do TITAN.</p>
      <div className="preview-actions"><button type="button" className="primary-action" onClick={confirmImport}>{importKind === 'cardio' ? 'Adicionar cardio ao projeto' : 'Ativar projeto importado'}</button><button type="button" className="secondary-action" onClick={()=>{setPreview(null);setSummary(null);setImportKind('project');}}>Cancelar</button></div>
    </div>}
  </section>;
}
...
function mergeCardioPlan(activePlan: TitanPlan, input: unknown): { ok: true; plan: TitanPlan } | { ok: false; errors: string[] } {
  ...
  if (cardio.goal !== 'first-5k') return { ok: false, errors: ['O Projeto Cardio precisa usar a meta first-5k.'] };
  ...
  const schedule = cardio.weeks.flatMap((rawWeek, weekIndex) => {
    if (!isRecord(rawWeek)) { errors.push(`Semana ${weekIndex + 1} é inválida.`); return []; }
    ...
    if (!Array.isArray(week.sessions) || week.sessions.length === 0) { errors.push(`Semana ${weekNumber} não possui sessões.`); return []; }
  ...
  return { ok: true, plan: {
    ...
    project: {
      ...project,
      cardioGoal: typeof cardio.name === 'string' && cardio.name.trim() ? cardio.name.trim() : 'Primeiros 5 km',
...
function toCardioSession(value: unknown, week: number, index: number, errors: string[]): TitanCardioSession[] {
  if (!isRecord(value)) { errors.push(`Semana ${week}, sessão ${index + 1} é inválida.`); return []; }
  ...
  if (!title || durationMinutes <= 0 || !CARDIO_TYPES.includes(rawType)) { errors.push(`Semana ${week}, sessão ${index + 1} possui dados inválidos.`); return []; }
  const day = inferDay(title, index);
  const description = readString(session.description);
  const target = readString(session.target);
  return [{ id: `${id}-w${week}`, day, startTime: '17:00', title: `Semana ${week} — ${title}`, type: rawType, durationMinutes, week, phase: `Semana ${week}`, goal: target || description || 'Progressão para 5 km', instructions: [description, target].filter(Boolean) }];
}
