...
export type ImportedPlanSummary = {
  workouts: number;
...
export function normalizeImportedPlan(plan: TitanPlan, sourceFile?: string): TitanPlan {
  ...
  const originalAuthor = plan.author?.trim() || 'Autor externo';
  return {
    ...plan,
    id: plan.id || crypto.randomUUID(),
    author: originalAuthor,
    description: [plan.description, `Importado para o TITAN em ${importedAt}. Origem externa preservada.`].filter(Boolean).join(' '),
    project: {
      ...(plan.project ?? { name: plan.name, objective: 'Projeto importado' }),
      source: 'imported',
      originalAuthor,
      importedAt,
