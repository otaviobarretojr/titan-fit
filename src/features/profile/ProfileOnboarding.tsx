...
type Props = { onComplete: () => void; onImportProject: () => void; onActivateDemo: () => void };

export function ProfileOnboarding({ onComplete, onImportProject, onActivateDemo }: Props) {
  ...
  if(step==='choice') return <main className="profile-onboarding"><section className="profile-hero"><span className="eyebrow">BEM-VINDO AO TITAN FIT</span><h1>Como você quer começar?</h1><p>Crie seu planejamento, use um projeto que já possui ou explore primeiro uma demonstração completa.</p></section><section className="profile-choice-grid"><button type="button" className="profile-choice-card primary" onClick={()=>setStep('profile')}><strong>Criar meu plano com o TITAN</strong><span>Informe seus dados e objetivos para preparar treino e cardio personalizados.</span></button><button type="button" className="profile-choice-card" onClick={onImportProject}><strong>Inserir meu projeto</strong><span>Use uma programação já criada e acompanhe execução, histórico e progresso no TITAN.</span></button><button type="button" className="profile-choice-card demo-choice" onClick={onActivateDemo}><strong>Explorar demonstração</strong><span>Abra o TITAN já preenchido com dados fictícios para conhecer treinos, cardio, evolução, Coach e demais recursos.</span><small>Os dados são somente de demonstração e podem ser removidos depois em Configurações.</small></button></section><small className="profile-privacy-note">Seus dados permanecem salvos localmente neste aparelho.</small></main>;
