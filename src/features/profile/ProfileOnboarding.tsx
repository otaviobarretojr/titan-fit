...
import type { CardioGoal, EquipmentAccess, PrimaryGoal, TrainingExperience } from './types';
...
export function ProfileOnboarding({ onComplete, onImportProject }: { onComplete: () => void; onImportProject: () => void }) {
  ...
  const [cardioGoal, setCardioGoal] = useState<CardioGoal>('health');
  ...
  async function saveProfile() {
    ...
    const trainingDays = Number(days);
    ...
    try {
      ...
      setError('Não foi possível salvar o perfil neste aparelho.');
    } finally {
      setSaving(false);
    }
  }
  ...
  if (step === 'choice') {
    return <main className="profile-onboarding">
      <section className="profile-hero">
        <span className="eyebrow">BEM-VINDO AO TITAN FIT</span>
        <h1>Como você quer começar?</h1>
        <p>Você pode criar um perfil para receber um planejamento personalizado ou inserir um projeto que já possui.</p>
      </section>
      <section className="profile-choice-grid">
        <button type="button" className="profile-choice-card primary" onClick={() => setStep('profile')}>
          <strong>Criar meu plano com o TITAN</strong>
          <span>Informe seus dados e objetivos para preparar treino e cardio personalizados.</span>
        </button>
        <button type="button" className="profile-choice-card" onClick={onImportProject}>
          <strong>Inserir meu projeto</strong>
          <span>Use uma programação já criada e acompanhe execução, histórico e progresso no TITAN.</span>
        </button>
      </section>
      <small className="profile-privacy-note">Seus dados permanecem salvos localmente neste aparelho.</small>
    </main>;
  }

  return <main className="profile-onboarding">
    <section className="profile-hero">
      <button type="button" className="profile-back" onClick={() => setStep('choice')}>← Voltar</button>
      <span className="eyebrow">PERFIL TITAN</span>
      <h1>Conte o essencial</h1>
      <p>Pedimos apenas informações que serão usadas no planejamento de treino e cardio.</p>
    </section>
    <section className="profile-form-card">
      <label>Como podemos te chamar?<input value={displayName} onChange={(e) => setDisplayName(e.target.value)} autoComplete="name" /></label>
      <label>Data de nascimento <small>Opcional nesta etapa</small><input type="date" value={birthDate} onChange={(e) => setBirthDate(e.target.value)} /></label>
      <div className="profile-field-row">
        <label>Altura (cm)<input type="number" inputMode="decimal" value={heightCm} onChange={(e) => setHeightCm(e.target.value)} /></label>
        <label>Peso (kg)<input type="number" inputMode="decimal" step="0.1" value={weightKg} onChange={(e) => setWeightKg(e.target.value)} /></label>
      </div>
      ...
      <div className="profile-field-row">
        <label>Dias de treino/semana<input type="number" min="1" max="7" value={days} onChange={(e) => setDays(e.target.value)} /></label>
        <label>Minutos por treino<input type="number" min="20" max="180" step="5" value={minutes} onChange={(e) => setMinutes(e.target.value)} /></label>
      </div>
      ...
      <button type="button" className="profile-save" onClick={() => void saveProfile()} disabled={saving}>{saving ? 'Salvando…' : 'Criar meu perfil'}</button>
    </section>
  </main>;
}
