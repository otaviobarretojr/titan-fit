...
export function ProjectManagementPanel({ onPlanActivated }: Props) {
  ...
  async function refresh() {
    ...
    setProjects(items); setProfile(currentProfile); setActiveProjectIdState(activeId); setLoading(false);
  }
  ...
  return <section className="settings-card project-manager" aria-label="Meus projetos">
    ...
    {!loading && projects.length === 0 && <p className="project-manager-empty">Seu primeiro projeto aparecerá aqui após importar ou gerar um plano.</p>}
