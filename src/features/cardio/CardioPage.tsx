...
import { CardioEvolutionPanel } from './CardioEvolutionPanel';
...
export function CardioPage({ plan, refreshKey = 0, onHistoryChange, initialSessionId, onCloseExecution }: Props) {
  ...
  return <section className="cardio-page"><header className="cardio-hero"><span className="eyebrow">CARDIO TITAN</span><h2>Condicionamento + 5 km</h2><p>Seu cardio programado começa pelo Dashboard. Aqui ficam o plano do dia e seu histórico de evolução.</p></header>{today?<section className="cardio-highlight-card"><div><span className="info-label">CARDIO DE HOJE</span><h3>{today.title}</h3><p>{today.startTime} · {today.durationMinutes} min · {cardioZoneLabel(today)}</p>{today.goal&&<p>{today.goal}</p>}</div><span className="cardio-pulse" aria-hidden="true">♡</span></section>:<section className="cardio-highlight-card"><div><span className="info-label">CARDIO DE HOJE</span><h3>Sem cardio programado</h3><p>Não há sessão de cardio prevista para hoje no projeto ativo.</p></div></section>}<CardioEvolutionPanel records={history} />{recent.length>0&&<section className="cardio-section"><div className="cardio-section-title"><div><span className="eyebrow">HISTÓRICO</span><h3>Últimas sessões</h3></div></div><div className="cardio-history-list">{recent.map((item)=><article key={item.id}><div><strong>{item.title}</strong><span>{formatDate(item.completedAt)} · {item.heartRate?`${item.heartRate} bpm`:'FC —'}</span></div><span>{formatDuration(item.durationSeconds)} · {formatDistance(item.distanceMeters)}{item.pace?` · ${item.pace}`:''}</span></article>)}</div></section>}</section>;
}
