import { loadCardioRecords } from '../cardio/storage';
...
export function CoachPage() {
  const report = createCoachReport(loadWorkoutHistory(), loadCardioRecords());

  return <>
    <section className="section-header">
      <span className="eyebrow">COACH TITAN</span>
      <h2>Leitura dos seus dados</h2>
      <p>Recomendações locais baseadas apenas nos registros disponíveis no TITAN FIT.</p>
    </section>
    ...
    <section className="coach-pillars" aria-label="Pilares do score">
      <article><span>Musculação</span><strong>{report.score.training}</strong></article>
      <article><span>Cardio</span><strong>{report.score.cardio}</strong></article>
    ...
    <section className="coach-disclaimer">
      ...
      <p>O score ainda não considera sono, nutrição, hidratação ou recuperação porque esses dados não existem no aplicativo nesta versão.</p>
    </section>
