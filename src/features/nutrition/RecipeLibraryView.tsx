...
import { TITAN_RECIPES, recipeMacros, reloadTitanRecipes, type RecipeDefinition } from './advanced';
...
export function RecipeLibraryView() {
  ...
  const baseRecipes = useMemo(() => TITAN_RECIPES.filter((recipe) => !recipe.id.startsWith('custom-recipe-')), []);
  const allRecipes = useMemo(() => [...recipes, ...baseRecipes], [recipes, baseRecipes]);
  ...
  function persistRecipe(recipe: RecipeDefinition, nextRecipes: RecipeDefinition[]) {
    saveCustomRecipes(nextRecipes); setRecipes(nextRecipes); reloadTitanRecipes();
  ...
  function saveRecipe() {
    ...
    persistRecipe(recipe, [recipe, ...recipes]);
    reset();
  }
  function duplicateRecipe(source: RecipeDefinition) {
    const recipe: RecipeDefinition = { id: createRecipeId(`${source.name} cópia`), name: `${source.name} — cópia`, description: source.description, ingredients: source.ingredients.map((ingredient) => ({ ...ingredient })) };
    persistRecipe(recipe, [recipe, ...recipes]);
  }
  function removeRecipe(recipe: RecipeDefinition) {
    ...
    saveCustomRecipes(next); setRecipes(next); reloadTitanRecipes();
