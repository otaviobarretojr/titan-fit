...
export function ExerciseLibraryPage() {
  ...
  return <section className="exercise-library-page" aria-label="Biblioteca TITAN de exercícios">
    <div className="library-hero"><div><span className="eyebrow">BIBLIOTECA TITAN</span><h3>Exercícios</h3><p>{TITAN_FULL_EXERCISE_CATALOG.length} exercícios estruturados para consulta e prescrição.</p></div><span className="library-count">{results.length}</span></div>
    ...
    <div className="library-filter-grid">
      <label>Grupo muscular<select value={muscle} onChange={(event) => setMuscle(event.target.value)}><option value="all">Todos</option>{EXERCISE_LIBRARY_MUSCLES.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
      <label>Nível<select value={experience} onChange={(event) => setExperience(event.target.value as TitanCatalogExercise['minExperience'] | 'all')}><option value="all">Todos</option><option value="beginner">Iniciante</option><option value="intermediate">Intermediário</option><option value="advanced">Avançado</option></select></label>
    </div>
    ...
    {results.length ? <div className="library-list">{results.map((exercise) => <button type="button" className="library-exercise-card" key={exercise.id} onClick={() => setSelected(exercise)}><div><strong>{exercise.name}</strong><small>{exercise.primaryMuscle}{exercise.secondaryMuscles.length ? ` · ${exercise.secondaryMuscles.join(', ')}` : ''}</small></div><div className="library-card-meta"><span>{exercise.repRange[0]}–{exercise.repRange[1]}</span><span>RIR {exercise.defaultRir}</span><span>{equipmentLabel(exercise.equipment[0])}</span></div><span className="programming-chevron">›</span></button>)}</div> : <div className="library-empty"><strong>Nenhum exercício encontrado</strong><p>Ajuste a busca ou os filtros.</p></div>}
  </section>;
}

function ExerciseDetail({ exercise, onBack }: { exercise: TitanCatalogExercise; onBack: () => void }) {
  return <section className="library-detail"><button type="button" className="secondary-action programming-back" onClick={onBack}>‹ Voltar à biblioteca</button><div className="library-detail-hero"><span className="eyebrow">{exercise.primaryMuscle.toUpperCase()}</span><h2>{exercise.name}</h2><p>{exercise.secondaryMuscles.length ? `Auxiliares: ${exercise.secondaryMuscles.join(', ')}` : 'Exercício focado no grupo principal.'}</p><div className="library-detail-metrics"><span><small>Repetições</small><strong>{exercise.repRange[0]}–{exercise.repRange[1]}</strong></span><span><small>RIR base</small><strong>{exercise.defaultRir}</strong></span><span><small>Descanso</small><strong>{exercise.restSeconds}s</strong></span></div></div>
  <div className="library-detail-grid"><Info title="Equipamento" value={exercise.equipment.map(equipmentLabel).join(' · ')} /><Info title="Nível mínimo" value={experienceLabel(exercise.minExperience)} /><Info title="Padrão" value={patternLabel(exercise.pattern)} /></div>
  <div className="library-guide-card"><span className="eyebrow">EXECUÇÃO</span><p>{exercise.technique}</p></div>
  <div className="library-guide-card"><span className="eyebrow">ERROS COMUNS</span><ul>{exercise.commonMistakes.map((item) => <li key={item}>{item}</li>)}</ul></div>
  {exercise.substitutions.length > 0 && <div className="library-guide-card"><span className="eyebrow">SUBSTITUIÇÕES</span><div className="library-substitutions">{exercise.substitutions.map((id) => { const substitute=TITAN_FULL_EXERCISE_CATALOG.find((item)=>item.id===id); return <span key={id}>{substitute?.name ?? id}</span>; })}</div></div>}
  </section>;
}
