...
export function ProgressPage({ refreshKey }: { refreshKey: number }) {
  ...
  return <>
    <section className="section-header evolution-center-header"><span className="eyebrow">CENTRO DE EVOLUÇÃO · v0.21</span><h2>Evolução</h2><p>Acompanhe o físico e o desempenho de treino no mesmo lugar.</p></section>
...
function WorkoutProgress({ refreshKey }: { refreshKey: number }) {
  ...
  const summary = useMemo(() => ({ totalVolumeKg: records.reduce((total, record) => total + (record.totalVolumeKg ?? 0), 0), totalSets: records.reduce((total, record) => total + (record.totalSets ?? 0), 0) }), [records]);
  ...
  return <div className="training-evolution-view">
    <section className="section-header"><span className="eyebrow">COACH TITAN</span><h2>Inteligência do treino</h2><p>PRs, progressão e recuperação estimada a partir do seu histórico real.</p></section>
...
function ExerciseIntelligenceCard({ exercise, sessions, records }: { exercise: HistoryExercise; sessions: number; records: WorkoutHistoryRecord[] }) {
  ...
  const pr = calculateStrengthPr(records, exercise.exerciseId); const advice = getProgressionAdvice(records, exercise.exerciseId); const latest = getExerciseSessions(records, exercise.exerciseId)[0]?.exercise;
  return <article className="progression-card coach-progress-card"><div><span className="info-label">{sessions} sessão{sessions === 1 ? '' : 'ões'} · MUSCULAÇÃO</span><h3>{exercise.name}</h3></div><div><span className="info-label">ÚLTIMA SESSÃO</span><strong>{latest ? formatLastStrengthSession(latest) : '—'}</strong></div><div className="history-metrics"><span>🏆 Carga: {pr.bestWeightKg ? `${pr.bestWeightKg} kg` : '—'}</span><span>🏆 Volume: {pr.bestSessionVolumeKg ? formatVolume(pr.bestSessionVolumeKg) : '—'}</span>{pr.bestSet && <span>🏆 Melhor série: {pr.bestSet.weightKg} × {pr.bestSet.repetitions}</span>}</div><div className={`coach-advice ${advice.status}`}><span className="info-label">COACH TITAN</span><strong>{advice.title}</strong><p>{advice.message}</p></div></article>;
}
