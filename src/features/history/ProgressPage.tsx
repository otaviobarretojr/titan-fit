...
export function ProgressPage({ refreshKey }: Props) {
  ...
  const exerciseProgress = useMemo(() => {
    const map = new Map<string, { name: string; sessions: number; bestWeightKg: number; latestWeightKg: number; latestAt: string }>();
  ...
  return <>
    ...
    <section className="progress-summary" aria-label="Resumo do histórico">
      <div><span className="info-label">TREINOS</span><strong>{records.length}</strong></div>
      <div><span className="info-label">SÉRIES</span><strong>{summary.totalSets}</strong></div>
      <div><span className="info-label">VOLUME</span><strong>{formatVolume(summary.totalVolumeKg)}</strong></div>
    </section>
