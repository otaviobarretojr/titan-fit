...
import { CardioExecutionView } from '../features/cardio/CardioExecutionView';
...
export function App() {
  ...
  function importPlan(plan: TitanPlan) { saveActivePlan(plan); setActivePlan(plan); setShowImporter(false); setDirectWorkoutId(null); setActiveCardioId(null); setActiveTab('today'); }
  function deletePlan() { if (!window.confirm('Remover o projeto ativo deste aparelho?')) return; removeActivePlan(); setActivePlan(null); setShowImporter(false); setDirectWorkoutId(null); setActiveCardioId(null); }
  function historyChanged() { setHistoryRefresh((value) => value + 1); setActiveTab('progress'); }
  function openTab(tab: TabId) { setActiveTab(tab); setActiveCardioId(null); if (tab !== 'plan') { setShowImporter(false); setDirectWorkoutId(null); } }
  function openPlan() { setShowImporter(!activePlan); setDirectWorkoutId(null); setActiveCardioId(null); setActiveTab('plan'); }
  function startWorkout(workoutId: string) { setShowImporter(false); setDirectWorkoutId(workoutId); setActiveCardioId(null); setActiveTab('plan'); }
  function startCardio(sessionId: string) { setActiveCardioId(sessionId); setDirectWorkoutId(null); setActiveTab('today'); }

  const activeCardio = activePlan?.project?.cardioSchedule?.find((session) => session.id === activeCardioId) ?? null;
  ...
  return <div className="app-shell">
    ...
    <main className="app-main">
      {activeCardio ? <CardioExecutionView session={activeCardio} onBack={() => setActiveCardioId(null)} onCompleted={() => { setActiveCardioId(null); setHistoryRefresh((value) => value + 1); }} /> : <>
        {activeTab === 'today' && <DashboardPage plan={activePlan} onOpenPlan={openPlan} onStartWorkout={startWorkout} onStartCardio={startCardio} onOpenProgress={() => openTab('progress')} />}
        ...
        {activeTab === 'more' && <><EmptyPage title="Configurações" body="Backup, instalação e atualização do seu aplicativo de treino." /><section className="settings-card" aria-label="Aplicativo"><div><span className="info-label">Versão</span><strong>v0.10.0</strong></div><div><span className="info-label">Engine de dados</span><strong>{dataEngineStatus === 'ready' ? 'Pronta' : dataEngineStatus === 'starting' ? 'Iniciando' : 'Indisponível'}</strong></div><div><span className="info-label">Conexão</span><strong>{isOnline ? 'Online' : 'Offline'}</strong></div><button type="button" className="secondary-action" onClick={installApp} disabled={!installPrompt}>{installPrompt ? 'Instalar aplicativo' : 'Instalação indisponível'}</button><button type="button" className="secondary-action" onClick={() => window.location.reload()}>Verificar atualização</button></section><BackupPanel /></>}
      </>}
    </main>
    ...
    {!activeCardio && <nav className="bottom-navigation" aria-label="Navegação principal">{tabs.map((tab) => <button key={tab.id} type="button" className={activeTab === tab.id ? 'active' : ''} onClick={() => openTab(tab.id)} aria-current={activeTab === tab.id ? 'page' : undefined}><span className="nav-icon" aria-hidden="true">{tab.icon}</span><span>{tab.label}</span></button>)}</nav>}
  </div>;
}
