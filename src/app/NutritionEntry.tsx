...
export function NutritionEntry() {
  ...
  function addRecipe(recipeId: string) {
    ...
    const additions = recipe.ingredients.map((ingredient, index) => ({ id: `${activeMeal.id}-${recipe.id}-${index}-${Date.now()}`, foodId: ingredient.foodId, plannedAmount: 0, actualAmount: ingredient.amount }));
  ...
  function editWeeklyAmount(dayId: string, mealId: string, itemId: string, amount: number) {
    const next = weeklyPlans.map((day) => day.id !== dayId ? day : { ...day, meals: day.meals.map((meal) => meal.id !== mealId ? meal : { ...meal, items: meal.items.map((item) => item.id === itemId ? { ...item, plannedAmount: Math.max(0, amount), actualAmount: Math.max(0, amount) } : item) }) });
    setWeeklyPlans(next); saveWeeklyPlan(next);
  }
  ...
  if (activeMeal) {
    ...
    return <main className="nutrition-app nutrition-meal-mode">
      ...
      {showFoodPicker && <MealFoodPicker onAdd={addFoodToMeal} onClose={() => { setShowFoodPicker(false); setReplaceItemId(null); }} replaceFoodId={replacingItem?.foodId} replaceAmount={replacingItem?.actualAmount} />}
  ...
  if (view === 'insights') {
    const coach = buildCoachMessage(history, plannedMacros.caloriesKcal || 2900, plannedMacros.proteinG || 195);
