...
export function NutritionEntry() {
  ...
  const [editingCompletedMealId, setEditingCompletedMealId] = useState<string | null>(null);
  ...
  const activeMealLocked = Boolean(activeMeal?.status === 'completed' && editingCompletedMealId !== activeMeal.id);
  ...
  function updateAmount(itemId: string, amount: number) {
    if (!activeMeal || activeMealLocked) return;
  ...
  function finishMeal(status: 'completed' | 'skipped') {
    ...
    setEditingCompletedMealId(null); setActiveMealId(null); setShowFoodPicker(false); setReplaceItemId(null);
  }
